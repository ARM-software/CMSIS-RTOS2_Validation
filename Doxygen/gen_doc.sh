...
# Version: 1.0
# Date: 2022-01-12
# This bash script generates CMSIS_RV2 documentation
#
# Pre-requisites:
...
function doxygen {
    ...
    if [[ $2 != 0 ]]; then
      ...
      cp -f "${DIRNAME}/_DoxyTemplates/search.css" "${DIRNAME}/html/search/"
    fi
        
    projectName=$(grep -E "PROJECT_NAME\s+=" $1 | sed -r -e 's/[^"]*"([^"]+)".*/\1/')
    projectNumber=$(grep -E "PROJECT_NUMBER\s+=" $1 | sed -r -e 's/[^"]*"([^"]+)".*/\1/')
    datetime=$(date -u +'%a %b %e %Y %H:%M:%S')
    sed -e "s/{datetime}/${datetime}/" "${DIRNAME}/_DoxyTemplates/footer.js" \
...
doxygen "${DIRNAME}/CMSIS_RV2.dxy" 1
exit 0
